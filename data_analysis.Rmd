...
*****note, will need to recode to numeric for the SEM (look up what needs to be done)******
```{r}
raw_data <- raw_data %>% 
...
Note, OSI variable is categorical (ordinal) not numeric
```{r}
raw_data %>% 
...
Can consider Cronbach's alpha for these (not for proportion or overall)
...
(can be done in the main dataframe just doing seperately for now...)
...
round(MuMIn::std.coef(AP_model, partial.sd = TRUE), 2) #to get standardised..
