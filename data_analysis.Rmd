...
# Visualise frequency variables

```{r}
read_frequency <- raw_data %>% dplyr::select(study_read_frequency, formal_read_frequency, class_read_frequency)
post_frequency <- raw_data %>% dplyr::select(study_post_frequency,formal_post_frequency, class_post_frequency)
share_frequency <- raw_data %>% dplyr::select(study_share_frequency, formal_share_frequency, class_share_frequency)

read_frequency_data <- pivot_longer(data = read_frequency, cols = c(study_read_frequency, formal_read_frequency, class_read_frequency), names_to = "group_type_1", values_to = "read")
post_frequency_data <- pivot_longer(data = post_frequency, cols = c(study_post_frequency,formal_post_frequency, class_post_frequency), names_to = "group_type_2", values_to = "post")
share_frequency_data <- pivot_longer(data = share_frequency, cols = c(study_share_frequency, formal_share_frequency, class_share_frequency), names_to = "group_type_3", values_to = "share")

together <- cbind(read_frequency_data, post_frequency_data, share_frequency_data)

together <- together %>% 
  mutate(group_type = case_when(group_type_1 == "study_read_frequency" ~ "Study AIMG",
                                  group_type_1 == "class_read_frequency" ~ "Class AIMG",
                                  group_type_1 == "formal_read_frequency" ~ "Formal AIMG")) %>% 
  dplyr::select(group_type, read, post, share)
```
...
use_frequency_figure <- raw_data %>% dplyr::select(study_read_frequency, formal_read_frequency, class_read_frequency,
  ...
  separate(context, into = c("group_type", "action_type", "other"), sep = "_") %>% 
  dplyr::select(group_type, action_type, frequency) %>% 
  mutate(frequency = factor(case_when(frequency == 0 ~ "Never",
                                      frequency == 0.25 ~ "A couple of times per semester",
                                      frequency == 0.5 ~ "A couple of times per week",
                                      frequency == 1 ~ "Daily", 
                                      frequency == 2 ~ "Multiple times per day"), 
